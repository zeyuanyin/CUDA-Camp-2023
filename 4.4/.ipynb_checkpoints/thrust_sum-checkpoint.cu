...
int main()
{
    ...
    thrust::copy(source, source+N, D.begin());

    cudaEvent_t start, stop_cpu, stop_gpu_naive, stop_gpu_2pass, stop_gpu_atomic, stop_gpu_shuffle, stop_thrust;
    ...
    int thrust_result = thrust::reduce(D.begin(), D.end(), 0, thrust::plus<int>());

    cudaEventRecord(stop_thrust);
    cudaEventSynchronize(stop_thrust);

    float time_cpu, time_gpu_naive, time_gpu_2pass, time_gpu_atomic, time_gpu_shuffle, time_thrust;
    ...
    cudaEventElapsedTime(&time_thrust, stop_cpu, stop_thrust);
    

    printf("CPU time: %.2f\nGPU naive time: %.2f\nGPU 2Pass time: %.2f\nGPU atomic time: %.2f\nGPU shuffle time: %.2f\nGPU thrust time: %.2f\n",time_cpu, time_gpu_naive/20, time_gpu_2pass, time_gpu_atomic/20, time_gpu_shuffle/20, time_thrust);
    ...
    printf("thrust Result: %s\nGPU_thrust_result: %d;\nCPU_result: %d;\n", (thrust_result == cpu_result)?"Pass":"Error", thrust_result, cpu_result);
